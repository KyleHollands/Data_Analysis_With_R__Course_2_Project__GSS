...
```
#### Conclusion
1. If p-value is small (less than α), reject H0. 
2. The data provide convincing evidence that at least 
one pair of population means are different from each 
other (but we can’t tell which one).

1. If p-value is large, fail to reject H0. 
2. The data do not provide convincing evidence that at 
...
on this analysis.
Additionally, there appears to be an outlier in the data, possible due to an 
entry error; further investigation into this is required.
